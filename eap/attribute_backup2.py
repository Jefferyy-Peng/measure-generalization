...
def get_scores_eap_ig(model: HookedTransformer, graph: Graph, dataloader: DataLoader, metric: Callable[[Tensor], Tensor], steps=30, quiet=False):
    ...
    for clean, corrupted, label in dataloader:
        ...
        batch_size = len(clean)
        total_items += batch_size
        clean_images = torch.stack(clean)
        corrupted_images = torch.stack(corrupted)
        ...
        (fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks), activation_difference = make_hooks_and_matrices(model, graph, batch_size, 197, scores)
        ...
        def input_interpolation_hook(k: int):
            def hook_fn(activations, hook):
                with torch.no_grad():
                    new_input = input_activations_corrupted + (k / steps) * (input_activations_clean - input_activations_corrupted).detach()
        ...
        try:
            ...
            for lst in (fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks):  # hooks = (fwd_cor, fwd_clean, bwd)
                lst.clear()
            del activation_difference, fwd_hooks_corrupted, fwd_hooks_clean, bwd_hooks
            gc.collect()  # <‑‑ forces ref‑count drop immediately
            torch.cuda.empty_cache()
            print(
                f"after cleanup – allocated:"
