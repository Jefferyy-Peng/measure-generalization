...
# ---- Plotting Setup ----
# baseline_cols = ["AC", "ANE", "EMD", "ATC", "rankme", "alphaReQ"]
baseline_cols = ["ATC", "EMD", "alphaReQ"]
...
task_2_title = {
    'PACS-set2': 'PACS',
    'camelyon17-set2': 'Camelyon17',
    'fmow-set2': 'FMoW'
}

metric_2_name = {
    'abs_srcc': 'Circuit Shift (Ours)',
    'EMD': 'MDE'
}
...
for row_idx, task in enumerate(tasks):
    ...
    for col_idx, metric in enumerate(circuit_metrics):
        ...
        #     ax.set_title(task)
        if row_idx == nrows - 1:
            ax.set_xlabel(metric_2_name[metric] if metric in metric_2_name.keys() else metric,fontsize=11)
        ax.set_ylabel(y_label[target_col],fontsize=11)
    ...
    for i, bcol in enumerate(baseline_cols):
        ...
        ax.text(0.05, 0.95, f"$R^2$={r2:.3f}\nrho={rho:.3f}\ntau={tau:.3f}", transform=ax.transAxes, va='top',
                ha='left', fontsize=10,
                bbox=dict(boxstyle="round,pad=0.3", edgecolor='gray', facecolor='white', alpha=0.8))
        if row_idx == nrows - 1:
            ax.set_xlabel(metric_2_name[bcol] if bcol in metric_2_name.keys() else bcol, fontsize=11)
        ax.set_ylabel(y_label[target_col],fontsize=11)
        if i == 0:
            ax.text(
                -0.26, 0.5,  # adjust -0.15 if too far/close
                task_2_title[task],
                fontweight="bold",
                fontsize=13,
                rotation=90,
...
plt.savefig(f"figures/domain_fit_plots{TOPN}_new.pdf")
# plt.show()
